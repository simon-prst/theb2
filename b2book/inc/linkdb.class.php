...
/* Data storage for links.
   ...
   Available keys:
       ...
       author : user who shared the link                   
...
class linkdb implements Iterator, Countable, ArrayAccess
{
    ...
    private $user; // If logged in, who is the current user ?
    ...
    // Input : $isLoggedIn : is the used logged in ?
    {
		...
		if ($this->loggedin) $this->user = $_SESSION['username'];
    ...
    private function checkdb() // Check if db directory and file exists.
    {
        ...
        {
             $this->links = array();
             $link = array('title'=>'Shaarli - sebsauvage.net','url'=>'http://sebsauvage.net/wiki/doku.php?id=php:shaarli','description'=>'Welcome to Shaarli ! This is a bookmark. To edit or delete me, you must first login.','private'=>0,'linkdate'=>'20110914_190000','tags'=>'opensource software', 'author'=>'admin');
             $this->links[$link['linkdate']] = $link;
             $link = array('title'=>'My secret stuff... - Pastebin.com','url'=>'http://pastebin.com/smCEEeSn','description'=>'SShhhh!!  I\'m a private link only YOU can see. You can delete me too.','private'=>1,'linkdate'=>'20110914_074522','tags'=>'secretstuff', 'author'=>'admin');
             $this->links[$link['linkdate']] = $link;
    ...
    private function readdb()
    {
		...
		{
            $toremove=array();
            foreach($this->links as $link) { if ($link['private']!=0 && $link['author']!=$this->user) $toremove[]=$link['linkdate']; }
            foreach($toremove as $linkdate) { unset($this->links[$linkdate]); }
        }
    ...
    // Case insentitive search among links (in url, title, author and description). Returns filtered list of links.
    ...
    public function filterFulltext($searchterms)
    {
        ...
        foreach($this->links as $l)
        {
            $found=   (strpos(strtolower($l['title']),$s)!==false)
                   ...
                   || (strpos(strtolower($l['author']),$s)!==false);
    ...
    // Filter by author.
    // You can specify one or more authors (they can be separated by space or comma).
    // eg. print_r($mydb->filterTags('mathieu simon'));
    public function filterAuthor($author)
    {
        $t = str_replace(',',' ',strtolower($author));
        $searchauthor=explode(' ',$t);
        $filtered=array();
        foreach($this->links as $l)
        {
            if (in_array($l['author'], $searchauthor))
                $filtered[$l['linkdate']] = $l;
        }
        krsort($filtered);
        return $filtered;
	...
	// Filter by like. like must be an integer between 1 and 5
    // Sort order is: older articles first.
    public function filterLike($like)
    {
        $filtered=array();
        foreach($this->links as $l)
        {
            if ($l['linkdate'] >= $like) $filtered[$l['linkdate']] = $l;
        }
        ksort($filtered);
        return $filtered;
    }
    ...
    // Filter by smallHash.
    // If the argument is a string, 1 article is returned
    // If the argument is an array, several articles are returned
    public function filterSmallHash($smallHash)
    {
        $filtered=array();
        if (is_array($smallHash)) {
            foreach($this->links as $l)
            {
                if (is_int(array_search(smallHash($l['linkdate']), $smallHash)))
                {
                    $filtered[$l['linkdate']] = $l;
                }
            }
